...
4. 取消勾选 `中英文扩展`；可选勾选 `同义词扩展`
5. 固定使用两条检索条件
   - 第 1 条：`主题 = query`
   - 第 2 条：`篇关摘 = query`
   - 两条条件之间逻辑为 `OR`
8. 可选设置起始/结束年份（与更新时间互斥）
9. 可选设置更新时间范围下拉框（与起止年份互斥）
10. 默认保留 `仅看有全文`
11. 传入参数后可取消 `仅看有全文`
12. 可选勾选核心来源
13. 提交检索
...
- `--date-range`：更新时间范围下拉框，与 `--date-from`/`--date-to` 互斥。可选值：
  - `week`：最近一周
  - `month`：最近一月
  - `half-year`：最近半年
  - `year`：最近一年
...
python cnki-search/scripts/cli.py advanced-search --query "机器学习" --date-range year --synonym-extend -n 200
python cnki-search/scripts/cli.py advanced-search --query "知识图谱" --date-range week --synonym-extend --core
```
...
- `date_to`
- `date_range`
- `date_range_type`（使用 `--date-range` 时返回，如 `最近一周`）
- `url`
- `file_path`
- `final_file_path`
- `intermediate_files`
