...
def create_parser() -> argparse.ArgumentParser:
    ...
    advanced_parser.add_argument("--date-range", choices=["week", "month", "half-year", "year", "ytd", "last-year"], help="更新时间范围（与 --date-from/--date-to 互斥）")
    advanced_parser.add_argument("--synonym-extend", action="store_true", help="勾选同义词扩展")
...
def _run_command_once(args: argparse.Namespace, config: CnkiSearchConfig) -> dict:
    ...
    try:
        ...
        if args.command == "advanced-search":
            if args.date_range and (args.date_from or args.date_to):
                raise ValueError("--date-range 与 --date-from/--date-to 不能同时传入")
