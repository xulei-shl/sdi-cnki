...
from cnki_yearly_progress_store import YearlySearchProgressStore
from exceptions import ValidationError
from progress_store import SearchProgressStore

logger = logging.getLogger("cnki_search.yearly_export")


class CnkiYearlyExportMixin:
    """负责截至年份全量导出的逐年外层编排。"""
    ...
    def _prepare_yearly_progress_store(
        ...
        return YearlySearchProgressStore.prepare_store(
            ...
            synonym_extend=bool(cli_params.get("synonym_extend")),
            date_range=cli_params.get("date_range"),
        )
    ...
    def _run_yearly_advanced_export(
        ...
        if yearly_result_files:
            final_file = output_dir / build_summary_output_filename(query=query, fallback="cnki-yearly")
    ...
    def _resolve_yearly_tasks(self, search_params: dict[str, Any], runtime: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return self._build_yearly_export_tasks(
            ...
            synonym_extend=bool(search_params.get("synonym_extend")),
    ...
    def _collect_available_end_years(self) -> list[str]:
        ...
        trigger = self.page.locator("div.sort.end .sort-default").first
        if trigger.count() > 0:
            try:
                trigger.click()
            except Exception as exc:
                logger.debug("展开结束年下拉失败，继续直接读取 DOM: %s", exc)
        options = self.page.locator("div.sort.end .sort-list a")
        years: list[str] = []
    ...
    def _build_yearly_sub_progress_file(self, output_dir: Path, task: dict[str, Any]) -> Path:
        ...
        return SearchProgressStore.build_default_path(
            ...
            core_only=task["core_only"],
            include_no_fulltext=task["include_no_fulltext"],
            synonym_extend=task.get("synonym_extend", False),
        ...
        while True:
            ...
            if retry_round > self.YEARLY_VALIDATION_MAX_RETRIES:
                mismatch_summary = ", ".join(
                    f"{item['year']}({item.get('reason', '数量不一致')})" for item in failures
                )
    ...
    def _write_yearly_summary_report(
        ...
        report_path = output_dir / build_summary_report_output_filename(query=query, fallback="cnki-yearly")
        lines = [
            f"检索词: {query}",
            f"状态: {status}",
            "模式: yearly",
        ...
        date_range_val = search_params.get("date_range")
        if date_range_val:
            label = CnkiFormMixin.DATE_RANGE_LABEL_MAP.get(date_range_val, date_range_val)
            lines.append(f"更新时间: {label}")
        lines.append(f"同义词扩展: {'是' if bool(search_params.get('synonym_extend')) else '否'}")
    ...
    def _build_yearly_result_payload(
        ...
        date_range_type = ""
        if date_range_val:
            date_range_type = CnkiFormMixin.DATE_RANGE_LABEL_MAP.get(date_range_val, date_range_val)
        return {
            ...
            "date_from": search_params.get("date_from"),
            "date_to": search_params.get("date_to"),
            "date_range": date_range_type or self._format_date_range(search_params.get("date_from"), search_params.get("date_to")),
