...
| CNKI 检索参数 | `query` | 字符串 | 是 | 检索词，实际执行两条条件：`主题=query OR 篇关摘=query` |
...
| 去重配置 | `dedup_scope_meta_task_id` | 外键 | 否 | 参考去重范围，选择其他任务模板的 ID，该模板下的所有历史数据参与去重比对（详见 4.2.2） |
...
- `dedup_scope_meta_task_id`：下拉选择框，普通用户仅显示自己创建的任务模板，管理员可看到所有模板。选择后检索结果不仅与当前模板的历史数据去重，同时也与所选参考模板的所有历史数据比对去重；不选择时仅执行当前模板内部的默认去重
...
2. 逐行处理：
   ...
   - 调用去重服务判断是否重复
   - 标记 `is_duplicate`（同一元任务内重复则为 `true`；若配置了参考去重范围 `dedup_scope_meta_task_id`，则同时与参考模板的所有历史数据比对重复）
...
def check_duplicate(title_norm: str, journal_norm: str, year: int,
                    ...
                    dedup_scope_meta_task_id: Optional[int] = None) -> Optional[int]:
    """
    返回：
      - None: 不重复
    ...
    for existing in existing_list:
        ...
        # 3. 参考去重范围（可选）→ 标记为重复
        if dedup_scope_meta_task_id is not None and \
           existing_instance.meta_task_id == dedup_scope_meta_task_id:
...
**配置位置**：管理员在「系统设置」页面填写 Webhook URL  
**存储表**：`system_configs` (`key = 'webhook_enterprise_wechat'`)
...
**注意**：所有用户的通知发送到同一个群，消息必须清晰标明**用户名**和**任务名**。
...
| `system_configs` | 系统配置 | `key`, `value` (Webhook、队列参数) |
...
CREATE TABLE meta_tasks (
    ...
    dedup_scope_meta_task_id INTEGER,                         -- 参考去重范围：关联 meta_tasks.id，该模板的所有历史数据参与去重比对
...
CREATE INDEX idx_meta_tasks_dedup_scope ON meta_tasks(dedup_scope_meta_task_id);
```

**说明**：LLM 配置改为多选优先级方式，通过中间表 `meta_task_llm_configs` 关联（见 A.5.1），不再直接存储 `llm_config_id`。
