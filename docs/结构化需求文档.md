...
2. 逐行处理：
   ...
   - 调用去重服务判断是否重复
   - 标记 `is_duplicate`（同一元任务内重复则为 `true`）
...
def check_duplicate(title_norm: str, journal_norm: str, year: int,
                    meta_task_id: int, current_instance_id: int) -> Optional[int]:
    """
    返回：
      - None: 不重复
      - task_result.id: 重复，返回被引用的记录 ID
    """
    # 1. 查询相同 (title_norm, journal_norm, year) 的已有记录
...
CREATE TABLE task_instances (
    ...
    status                  VARCHAR(20) DEFAULT 'pending' CHECK(
        status IN ('pending', 'running', 'search_completed',
                   ...
                   'ready_for_download', 'downloading', 'completed', 'failed', 'cancelled')
    ),
