...
| | `query_group_a` | 字符串数组 | 条件必填 | `professional` 模式下主题A关键词组（≥1个），组内关键词用 `+` 连接（OR 逻辑） |
| | `query_group_b` | 字符串数组 | 条件必填 | `professional` 模式下主题B关键词组（≥1个），组内关键词用 `+` 连接（OR 逻辑） |
...
| **专业检索** (`professional`) | `search_mode = 'professional'` | 切换到「专业检索」Tab，填入布尔表达式 `(SU=(A1+A2...) AND SU=(B1+B2...)) OR (TKA=(A1+A2...) AND TKA=(B1+B2...))` | **1 次** |
...
- `professional` 模式：`query_group_a` / `query_group_b` 分别为两个主题域的关键词组，每组内关键词用 OR 连接。系统自动生成 CNKI 专业检索表达式 `(SU=(A1+A2...) AND SU=(B1+B2...)) OR (TKA=(A1+A2...) AND TKA=(B1+B2...))`，**仅执行 1 次检索**，大幅减少检索次数
