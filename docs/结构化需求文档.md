...
| **专业检索** (`professional`) | `search_mode = 'professional'` | 切换到「专业检索」Tab，填入布尔表达式：二者皆有时 `(SU=(A1+A2...) AND SU=(B1+B2...)) OR (TKA=(A1+A2...) AND TKA=(B1+B2...))`；仅一组时 `SU=(K1+K2...) OR TKA=(K1+K2...)` | **1 次** |
...
- `professional` 模式：`query_group_a` / `query_group_b` 分别为两个主题域的关键词组，每组内关键词用 OR 连接。二者皆有时生成 `(SU=(A1+A2...) AND SU=(B1+B2...)) OR (TKA=(A1+A2...) AND TKA=(B1+B2...))`；仅有一组时生成 `SU=(K1+K2...) OR TKA=(K1+K2...)`（字段覆盖不变，只是去掉两组间的 AND 约束）。**仅执行 1 次检索**，大幅减少检索次数
