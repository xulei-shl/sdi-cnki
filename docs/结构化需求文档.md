...
| **专业检索** (`professional`) | `search_mode = 'professional'` | 切换到「专业检索」Tab，填入布尔表达式：二者皆有时 `(SU=(A1+A2...) AND SU=(B1+B2...)) OR (TKA=(A1+A2...) AND TKA=(B1+B2...))`；仅 A 时 `SU=(A1+A2...)`；仅 B 时 `TKA=(B1+B2...)` | **1 次** |
...
- `professional` 模式：`query_group_a` / `query_group_b` 分别为两个主题域的关键词组，每组内关键词用 OR 连接。二者皆有时生成 `(SU=(A1+A2...) AND SU=(B1+B2...)) OR (TKA=(A1+A2...) AND TKA=(B1+B2...))`；仅 `query_group_a` 时生成 `SU=(A1+A2...)`；仅 `query_group_b` 时生成 `TKA=(B1+B2...)`。**仅执行 1 次检索**，大幅减少检索次数
