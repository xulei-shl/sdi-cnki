"""CNKI advanced search interactor — faithful port of docs/cnki-search/scripts/ mixins."""
...
logger = logging.getLogger("cnki.interactor")
...
class CnkiInteractor:
    """Sync CNKI interactor — runs in thread pool via asyncio.to_thread."""
    ...
    # ═══════════════════════════════════════════════════════════
    #  FORM — faithful to CnkiFormMixin
    # ═══════════════════════════════════════════════════════════
    ...
    def _ensure_advanced_condition_rows(self, required_count: int) -> None:
        ...
        while _time.time() < deadline:
            ...
            if rows.count() >= required_count:
                return
            if not self._click_first_available(["#gradetxt a.add-group", "#gradetxt .add-group", "a.add-group"]):
                break
            _time.sleep(0.3)
        if self.page.locator("#gradetxt dd").count() < required_count:
    ...
    def _set_year_input_value(self, selectors: list[str], value: str) -> None:
        """Exact JS from prototype — CNKI relies on specific attributes + events."""
    ...
    # ═══════════════════════════════════════════════════════════
    #  NAVIGATION — faithful to CnkiNavigationMixin
    # ═══════════════════════════════════════════════════════════

    def _is_advanced_search_page(self) -> bool:
        for sel in ["#gradetxt", "input[placeholder='结束年']", "#onlyfulltext", "input.btn-search"]:
            try:
    ...
    # ═══════════════════════════════════════════════════════════
    #  SELECTION — faithful to CnkiSelectionMixin
    # ═══════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════
    #  EXPORT — faithful to CnkiExportMixin
    # ═══════════════════════════════════════════════════════════
    ...
    def _export_batch(self, query: str, batch_index: int) -> tuple[Path, Path]:
    ...
    def _merge_final(self, batch_files: list[dict], query: str) -> Path:
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        slug = self._slug(query)[:40]
    ...
    def execute_search(self, params: dict) -> dict:
        ...
        print(f"[CNKI] Starting search: {query}")
        ...
        if not self._is_advanced_search_page():
            raise NavigationStateError("打开高级检索页面失败")
        ...
        total = summary["total"]
        print(f"[CNKI] Total results: {total}")
        if total == 0:
        ...
        merged_path = self._merge_final(batch_files, query)
        print(f"[CNKI] Complete: {merged_path}")
        self.browser.save_session()
        return {
