...
async def run_llm_analysis(
    ...
    try:
        ...
        task_results = list(r.scalars().all())
        ...
        for i in range(0, total, BATCH_SIZE):
            ...
            coros = [_process_one(db, tr, db_configs, prompt_template, instance_id) for tr in batch]
            batch_results = await asyncio.gather(*coros)

            for success, err in batch_results:
                if success:
                    analyzed += 1
                else:
                    failed += 1
...
async def _process_one(
    ...
    """Process a single task result through LLM. Returns (success, error_message)."""
    try:
        await db.execute(
